    ...
    <div id="order" class="contents-main">
        ...
        <table class="form">
			...
			<tr>
                <th>利用ホーム</th>
                <td>
                    <!--{assign var=key1 value="order_home"}-->
                    <span class="attention"><!--{$arrErr[$key1]}--></span>
                    <span style="<!--{$arrErr[$key1]|sfGetErrorColor}-->">
                    <!--{html_radios name=$key1 options=$arrHome selected=$arrForm[$key1].value}-->
                    </span>
                </td>
            </tr>
			<tr>
                <th>介護度</th>
                <td>
                    <!--{assign var=key1 value="order_care"}-->
                    <span class="attention"><!--{$arrErr[$key1]}--></span>
                    <span style="<!--{$arrErr[$key1]|sfGetErrorColor}-->">
                    <select name="<!--{$key1}-->">
	                    <option value="" selected="selected">選択してください</option>
	                    <!--{html_options options=$arrCare selected=$arrForm[$key1].value}-->
                    </select>
                    </span>
                </td>
            </tr>
