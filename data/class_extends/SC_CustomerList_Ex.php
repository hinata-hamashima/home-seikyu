...
class SC_CustomerList_Ex extends SC_CustomerList
{
	public function __construct($array, $mode = '')
    {
		...
		// 利用者ホーム
        if (!isset($this->arrSql['search_home'])) $this->arrSql['search_home'] = '';
        if (is_array($this->arrSql['search_home'])) {
            $arrHomeVal = $this->setItemTerm($this->arrSql['search_home'], 'home');
            foreach ($arrHomeVal as $data) {
                $this->arrVal[] = $data;
            }
        }
