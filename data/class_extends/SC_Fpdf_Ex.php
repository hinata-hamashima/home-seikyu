...
class SC_Fpdf_Ex extends SC_Fpdf
{
	public function __construct($download, $title, $tpl_pdf = 'nouhinsyo1.pdf')
    {
        ...
        $this->width_cell = array(90.3,10,35.10,35.5);

        $this->label_cell[] = 'サービス内訳';
        $this->label_cell[] = '数量';
        $this->label_cell[] = '単価';
    ...
    private function setShopData()
    {
        ...
        $this->lfText(158, 74, $arrInfo['law_addr02'], 8);
		...
		*/
        ...
        //ロゴ画像
        $logo_file = PDF_TEMPLATE_REALDIR . 'log.png';
        $this->Image($logo_file, 124, 40, 40);
		
		//会社印
		//$mark_file = PDF_TEMPLATE_REALDIR . 'mark.jpg';
        //$this->Image($mark_file, 165, 65, 20);		
    }
    ...
    private function setOrderData()
    {
        ...
        $this->lfText(36, $y + 2, $text, 25); //購入者氏名

        // お届け先情報
        $this->SetFont('SJIS', '', 10);
        //$this->lfText(25, 125, SC_Utils_Ex::sfDispDBDate($this->arrDisp['create_date']), 10); //ご注文日
        $this->lfText(130, 102, $this->arrDisp['order_id'], 15); //ご利用番号

        $this->SetFont('Gothic', 'B', 15);
        $this->Cell(0, 12, $this->tpl_title, 0, 2, 'C', 0, '');  //文書タイトル（納品書・請求書）
        $this->Cell(0, 66, '', 0, 2, 'R', 0, '');
        $this->Cell(5, 0, '', 0, 0, 'R', 0, '');
        $this->SetFont('SJIS', 'B', 20);
        $this->Cell(68, 7, number_format($this->arrDisp['payment_total']).' 円', 0, 5, 'R', 0, '');
